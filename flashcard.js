...
if (cardType === "Basic Card") {
   inquirer.prompt([
       ...
       fs.writeFile("flashcardLibrary.json", JSON.stringify(library, null, 2)); //write the updated array to the carLibrary.json file
       ...
       inquirer.prompt([					//use inquirer to ask if the user wants to keep making cards
           ...
           if (appData.anotherCard === "Yes") {	//If 'Yes' then..
               ...
               setTimeout(letsStart, 1000);			//reopen the main menu to the user
           }
       });
   });
   ...
   inquirer.prompt([
       ...
       if (cardObj.text.indexOf(cardObj.cloze) !== -1) {   //checking to make sure the Cloze matches some text in the statement
           library.push(cardObj);							//push the new card into the array of cards
           fs.writeFile("flashcardLibrary.json", JSON.stringify(library, null, 2)); //write the updated array to the cardLibrary file
       ...
       inquirer.prompt([					//use inquirer to ask if the user wants to keep making cards
           ...
           if (appData.anotherCard === "Yes") {	//If 'Yes' then..
               ...
               setTimeout(letsStart, 1000);		//return the user to the open menu
           }
       });
   });
}
...
letsStart();			//call the menu for the user to continue using the app
}
};
...
fs.writeFile("flashcardLibrary.json", JSON.stringify(newDeck, null, 2)); //write the new randomized array over the old one
console.log(colors.cyan("The deck of flashcards have been shuffled"));
//setTimeout(, 1000);  //*** shuffle only works on app reload, look into how to apply it in-app
}
